...
struc_file = 'relaxed_PdCoO2.in'
forceconstants_file = 'FORCE_CONSTANTS'
primitive_cell = [[1,0,0],[0,1,0],[0,0,1]]
#primitive_cell = 'auto'
supercell = [2,2,2] 
Q_start = np.array([3.33333333,3.33333333,3.33333333])
Q_end = np.array([4,3,3])
Q_steps = 201
Temperature = 300
coh_scatter_length ={'Pd': 4.6, 'Co': 2.7, 'O': 1.6}
#coh_scatter_length ={"Pd": 5.91, "Co":2.49, "O":5.803}
E_min = 0
E_max = 30
E_step = 0.02
xlabels = 'H 0 10'
savefigas = 'H010_sqe.png'
colormax = 1
colormin = -1
#broadening_factor = 4 ## I'm using a gaussian filter to do 2D broadening. 1/2 is good number
...
if __name__ == '__main__':
    ...
    np.savetxt('H010_sqe.txt',BinnedSQE.T)
    # broadening
    #sigma = broadening_factor
    #temporary = ndimage.gaussian_filter(BinnedSQE,sigma,mode='reflect')
    ...
    ax.set_xticklabels([np.around(q_start,decimals=2),np.around(q_end,decimals=2)])
